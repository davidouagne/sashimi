...
- **No fixed/pattern values are baked into the Backbone Element children.** `StructureDefinitionMapper` only declares `.isPrimaryKey`, `.uniqueKeyName`, `.precision`, `.scale`, `.fkN.targetColumn` with their type and cardinality — it never emits an FSH Assignment Rule or `^fixed[x]` for them (unlike the extension-based form's `^extension[=].value...`, which did bake in the concrete value). This is a deliberate consequence of the migration, not an oversight: see `docs/adr/0001-backbone-element-over-extensions-for-column-facts.md`.
